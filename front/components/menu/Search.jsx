...
//      <statnTid>1005000546</statnTid> 다음
        console.log('방면 맞나?',data.trainLineNm)
        // const curr = subway.filter(item => (item.subwayId === data.subwayId && item.updnLine === data.updnLine && item.trainLineNm.split(' - ')[1] === data.trainLineNm.split(' - ')[1]))
        // console.log(curr)
        const statnFid = subway.filter(item => item.statnId === data.statnFid)
        const statnTid = subway.filter(item => item.statnId === data.statnTid)
